...
public class JChatFrameMultipleWindowsWithSendButtonWidthByHeight_backup extends JChatFrame {
    ...
    public void testTForDebugTrigger(String s){
         ...
         if(s.startsWith(DefaultConversationController.fakeTypingByClientSTARTTrigger))  dr.startFakeKeyboardActivity();
         else if(s.startsWith(DefaultConversationController.fakeTypingByClientSTOPTrigger)) dr.stopFakeKeyboardActivity();
            //System.exit(-234);
    }
...
public class DoScrolling implements Runnable{
  ...
  public class DoAppendTextWithCaretCheck implements Runnable{
          ...
          public void run(){
              ...
              if(isbyself){
                   jcfmwwsbw.getClientEventHandler().reportInterfaceEvent(diet.client.ClientInterfaceEventTracker.chatwindow_appendbyself_clearingtextentry, timeDisplayed,  av1);     
              }
              else{
                   jcfmwwsbw.getClientEventHandler().reportInterfaceEvent(diet.client.ClientInterfaceEventTracker.chatwindow_appendbyotherparticipant, timeDisplayed  ,av1, av2, av3);      
              }
