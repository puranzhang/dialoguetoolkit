...
import diet.client.ClientInterfaceEventTracker;
...
public class Conversation extends Thread{
    ...
    private void doBasicSetup(String nameOfConversationController){
        ...
        cH= new ConversationHistory(this,  cC.param_experimentID, convIO);
    ...
    public synchronized boolean addNewParticipant(Participant p){
      ...
      try{
        ...
        if(this.expManager.emui!=null){
           this.expManager.emui.println("Main", "Participant with email: "+p.getParticipantID()+" and with username: "+p.getUsername()+" has logged in to "+cC.param_experimentID+" there are "+ps.getAllParticipants().size());
        }
      }catch (Exception e){
    ...
    public void run(){
        ...
        while (isConversationActive()){
          try{
             ...
             if(DefaultConversationController.debugTime)this.saveTime("1");
             ...
             if(DefaultConversationController.debugTime)this.saveTime("2");
             if (m!=null){
               if(DefaultConversationController.debugTime)this.saveTime("3"+m.getClass().toString());
               cHistoryUIM.updateControlPanel(m);
               if(DefaultConversationController.debugTime)this.saveTime("4");
               convIO.saveMessage(m);
               if(DefaultConversationController.debugTime)this.saveTime("5");
               ...
               Participant origin = ps.findParticipantWithEmail(m.getEmail());
                if(DefaultConversationController.debugTime)this.saveTime("6");
               ...
               //System.out.println("UPDATINGCONTROLPANEL");
                ...
                if (m instanceof MessageChatTextFromClient) {
                    if(DefaultConversationController.debugTime)this.saveTime("7");
                   ...
                   // System.out.println("MCT1");
                    ...
                    if(DefaultConversationController.debugTime)this.saveTime("8"); 
                    ...
                    if(DefaultConversationController.debugTime)this.saveTime("9"); 
                    ...
                    if(!msctfc.hasBeenRelayedByServer){
                         ...
                         this.setNewTurnBeingConstructedNotRelayingOldTurnAddingOldTurnToHistory(origin, msctfc, "NOTRELAYED", vAdditionalValues);
                         if(m instanceof MessageChatTextFromClient && diet.debug.Debug.debugtimers ){    
                           ((MessageChatTextFromClient)m).saveTime("serverConversation.hasBeenSavedBecauseNotrelayed");
                         }
                    
                    }
                    if(DefaultConversationController.debugTime)this.saveTime("10"); 
                    ...
                    if(DefaultConversationController.debugTime)this.saveTime("11"); 
                    ...
                    if(DefaultConversationController.debugTime)this.saveTime("12"); 
                    ...
                    try{
                       ...
                       this.deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns("POPUPRECEIVED:"+popupID, participantID ,username, username,mpr.getTimeOfSending().getTime() ,mpr.getTimeOfReceipt().getTime(), mpr.getTimeOfReceipt().getTime(),(title+"/"+question+"/"+optionsFLATTENED).replaceAll("\n","(NEWLINE)")+ timeOnClientOfShowing+timeOnClientOfChoice+"_"+mpr.getSelection()+"__"+mpr.getSelectedValue(), null);                        
          ...
          if(DefaultConversationController.debugMESSAGEBLOCKAGE){System.out.println("MCT44");System.out.flush();}
          }catch (Exception e){
               System.err.println(cC.param_experimentID+ ": ERROR SOMEWHERE IN THE CONVERSATION CONTROLLER");
       ...
       if(DefaultConversationController.debugMESSAGEBLOCKAGE){System.out.println("MCT45");System.out.flush();}
      }
         if(DefaultConversationController.debugMESSAGEBLOCKAGE){System.out.println("MCT46");System.out.flush();}
    ...
    public void newsendArtificialTurnFromApparentOrigin( Participant apparentOrigin, Participant recipient, String text){  
        ...
        int windowNo = cC.getStyleManager().getWindowNumberInWhichParticipantBReceivesTextFromParticipantA(apparentOrigin, recipient);
      ...
      public void newsendArtificialTurnFromApparentOrigin( Participant apparentOrigin, Participant recipient, String text, int windowNo, Vector additionalValues){  
        ...
        String textWithNoEnters = text.replace("\n", DefaultConversationController.spreadsheetOutputNEWLINESTRING).replace("\r", DefaultConversationController.spreadsheetOutputNEWLINESTRING);
...
//
    ...
    public void showPopupOnClientQueryInfo(String popupID,Participant recipient, String question, String[] options, String title, int selection, String prefixFORCSVSpreadsheet){
        ...
        this.deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(prefixFORCSVSpreadsheet+"POPUPSENT", recipient.getParticipantID(), recipient.getUsername(), recipient.getUsername(),new Date().getTime() ,new Date().getTime(), new Date().getTime(),titleCleaned+"-"+questionCleaned, null);
    ...
    public void deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(Participant recipient, String subdialogueID, String data){
        this.newsaveAdditionalRowOfDataToSpreadsheetOfTurns(subdialogueID,"data"   , "Server", "server", recipient.getParticipantID()+"."+recipient.getUsername(),new Date().getTime() ,new Date().getTime(), new Date().getTime(),data, null);
    ...
    public void deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(Participant sender,String recipientName , long timeCREATEDONCLIENT, String subdialogueID, String data){
        //String recipient
        this.newsaveAdditionalRowOfDataToSpreadsheetOfTurns(subdialogueID, "data",  sender.getParticipantID(), sender.getUsername(), recipientName, timeCREATEDONCLIENT,new Date().getTime(), new Date().getTime(),data, null);
    ...
    public void deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(String subDialogueID, String data){    
        ...
        Conversation.this.newsaveAdditionalRowOfDataToSpreadsheetOfTurns(subDialogueID, "data", "server", "server", "server",timeOfCreationOnServer ,timeOfCreationOnServer, timeOfCreationOnServer, data, new Vector());
     ...
     public void deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(String subdialogue, String datatype, String senderID, String senderUsername, long timeOfCreationOnClient,long timeOfSendOnClient, long timeOfRELAYONSERVER, String text, Vector additionalData){
        try{
            newsaveAdditionalRowOfDataToSpreadsheetOfTurns( subdialogue, datatype,  senderID, senderUsername, "", timeOfCreationOnClient , timeOfSendOnClient, timeOfRELAYONSERVER,  text, additionalData);
    ...
    public void newsaveAdditionalRowOfDataToSpreadsheetOfTurns(String subdialogueID, String datatype,  String senderID, String senderUsername, String recipient,long timeOfCreationOnClient ,long timeOfSendOnClient, long timeOfRELAYONSERVER, String text, Vector additionalData){
        try{
            
           cH.saveDataAsRowInSpreadsheetOfTurns(subdialogueID, datatype,timeOfCreationOnClient ,timeOfSendOnClient, timeOfRELAYONSERVER ,senderID, senderUsername, recipient,text.replaceAll("\n", "(NEWLINE)"),new Vector(),false,new Vector(), new Vector(), additionalData, false);
             }catch (Exception e){
    ...
    public void newsaveAdditionalRowOfDataToSpreadsheetOfTurns(String datatype,  Participant p, String value){
        try{
           ...
           cH.saveDataAsRowInSpreadsheetOfTurns(subDialogueID, datatype,timeOfCreationOnClient ,timeOfSendOnClient, timeOfRELAYONSERVER ,senderID, senderUsername, recipient,text.replaceAll("\n", "(NEWLINE)"),new Vector(),false,new Vector(), new Vector(), new Vector(), false);
           }catch (Exception e){
            e.printStackTrace();
        }
    }
    ...
    public void newsaveClientDocumentchangeToFile(Participant sender,long timeOfCreationOnClient ,long timestamp, String contentsOfWindow){
        String subdialogueID = cC.pp.getSubdialogueID(sender);
        convIO.saveClientDocumentChange(cC.param_experimentID, subdialogueID, sender, timeOfCreationOnClient, timestamp, contentsOfWindow);
    }
      ...
      public void newrelayTurnToParticipant(Participant sender,Participant recipient, MessageChatTextFromClient mct,  MutableAttributeSet style, int windowNumber, String subdialogueID, Vector additionalValues){
        ...
        ps.sendMessageToParticipant(recipient, mctc);
        cH.saveMessageRelayedToOthers(subdialogueID,mct.getStartOfTypingOnClient(),  mct.getTimeOfSending().getTime(),mct.getTimeOfReceipt().getTime(),sender.getParticipantID(), sender.getUsername(),sender.getUsername(),mct.getText(),pUsernames,false,mct.getKeypresses(),ds.getAllInsertsAndRemoves(),additionalValues,false);
      ...
      public void newrelayTurnToMultipleParticipants(Participant sender,Vector recipients, MessageChatTextFromClient mct, Vector additionalValues){
        ...
        for(int i=0;i<recipients.size();i++){
            Participant recipient = (Participant)recipients.elementAt(i);
            int windowNumber = cC.sm.getWindowNumberInWhichParticipantBReceivesTextFromParticipantA(sender, recipient);
            pUsernames.addElement(recipient.getUsername());  
             MutableAttributeSet style = cC.getStyleManager().getStyleForRecipient(recipient, sender);
            MessageChatTextToClient mctc = new MessageChatTextToClient(generateNextIDForClientDisplayConfirm(),sender.getParticipantID(),sender.getUsername(),mct.getText(),windowNumber,true,style);
            ps.sendMessageToParticipant(recipient, mctc);
      ...
      public void newrelayTurnToPermittedParticipants(Participant sender, MessageChatTextFromClient mct){
        ...
        cH.saveMessageRelayedToOthers(subdialogueID,mct.getStartOfTypingOnClient(),  mct.getTimeOfSending().getTime(),mct.getTimeOfReceipt().getTime(),sender.getParticipantID(), sender.getUsername(),sender.getUsername(),mct.getText(),pUsernames,false,mct.getKeypresses(),ds.getAllInsertsAndRemoves(),new Vector(),false);
      }
   ...
   // }
    ...
    public void typingactivity_ClearAllFakeTypingActivity(Participant p){
        cC.itnt.addSpoofNotTypingInfo(p, new Date().getTime());
    }
    ...
    /**
     ...
     public void setNewTurnBeingConstructedNotRelayingOldTurnAddingOldTurnToHistory(Participant p,MessageChatTextFromClient turnNotRelayed, String subdialogueID, Vector additionalValues){
        ...
        cH.saveInterceptedNonRelayedMessage(subdialogueID, turnNotRelayed.getStartOfTypingOnClient(), turnNotRelayed.getTimeOfSending().getTime(), turnNotRelayed.getTimeOfReceipt().getTime(), 
                p.getParticipantID(), p.getUsername(), "", turnNotRelayed.getText(), new Vector(), turnNotRelayed.isHasBeenBlocked(), turnNotRelayed.getKeypresses(), ds.getAllInsertsAndRemoves(), additionalValues,false);
...
//public String redbackground = "<html><head><style type=\"text/css\">body {color: white; background: red;}div { font-size: 200%;}</style></head><body><div>INCORRECT </div></body></html>";
    ...
    public void changeJProgressBarsOfAllParticipants(String id, String text, Color foreCol,int value){
        if(DefaultConversationController.debugMESSAGEBLOCKAGE){System.out.println("SINH1");;System.out.flush();}
        Vector v = ps.getAllParticipants();
         if(DefaultConversationController.debugMESSAGEBLOCKAGE){ System.out.println("SINH2");;System.out.flush();}
        for(int i=0;i<v.size();i++){
            
            Participant p = (Participant)v.elementAt(i);
            if(DefaultConversationController.debugMESSAGEBLOCKAGE){ System.out.println("SINH4 "+p.getUsername());;System.out.flush();}
             ps.changeJProgressBar(p, id, text, foreCol, value);
             if(DefaultConversationController.debugMESSAGEBLOCKAGE){System.out.println("SINH5 "+p.getUsername());System.out.flush();}
        }
        if(DefaultConversationController.debugMESSAGEBLOCKAGE){System.out.println("SINH6 ");System.out.flush();}
    }
