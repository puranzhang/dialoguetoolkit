...
public class IntelligentIO {
    ...
    String separator = DefaultConversationController.spreadsheetOutputSeparator;
    Conversation c;
    ...
    public void writeToObjectFileCreatingIfNecessary(String filename, Object o){
             if(DefaultConversationController.debugTime)c.saveTime("5A");
             IntelligentObjectFileWriter iofw = (IntelligentObjectFileWriter) this.htObjectFiles.get(filename);
             if(DefaultConversationController.debugTime)c.saveTime("5B");
             if(iofw==null){
                 iofw = new IntelligentObjectFileWriter(new File(conversationDirectory,filename));
                 this.htObjectFiles.put(filename, iofw);
                  if(DefaultConversationController.debugTime)c.saveTime("5BBBBBBBBBB");
             }
             if(DefaultConversationController.debugTime)c.saveTime("5C");
             iofw.saveObject(o);  
             if(DefaultConversationController.debugTime)c.saveTime("5D");
    }
    ...
    public void saveTurnAsAttribVals(Turn t){
         String[] header = DefaultConversationController.headerForSpreadsheetOutputFile; 
         if(t instanceof NormalTurnProducedByParticipant){
    ...
    public void saveErrorLog(String s){
     ...
     public void saveClientKeypressFromClient(MessageKeypressed mkp, String subDialogueID, String sender){
        ...
        String line = "";
        line = line +c.getController().param_experimentID + separator;
