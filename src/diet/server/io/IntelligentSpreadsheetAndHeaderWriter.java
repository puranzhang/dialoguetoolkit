        ...
        for(int i=0;i<this.vAttributeIDs.size();i++){
            ...
            for(int j=0;j <v.size();j++){
                ...
                if(av.getID().equalsIgnoreCase(s)){
                    ...
                    row = row + av.getValAsString()   + DefaultConversationController.spreadsheetOutputSeparator;
                    break;
                }
            }
            if(!found){
                row = row    + DefaultConversationController.spreadsheetOutputSeparator;
            }      
        }
